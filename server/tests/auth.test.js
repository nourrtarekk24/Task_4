...
describe('Authentication controller integration', () => {
  ...
  */
  test('authenticates the same user and issues a fresh JWT', async () => {
    // Perform a login using the credentials we registered earlier.
    ...
    // Successful login should return 200, provide a token and the public
    ...
    // Store the token so subsequent tests can authenticate as this user.
    issuedToken = payload.token;
  });
